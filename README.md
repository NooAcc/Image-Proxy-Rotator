...
| 请求统计 | 走代理的请求总数、成功率、平均耗时、有多少次归不到具体节点 |
| 节点使用分布 | 哪个节点在干活、各自占比多少、成功率如何 |
...
- 计数**跨浏览器重启累计**，存在本机 `chrome.storage.local`。想归零点「清零统计」——
  它只动统计数字，不碰节点、规则和设置。
- 只统计**命中规则、本该走代理**的请求；直连的请求不计入。成功的判定是 HTTP 状态码小于 400。
- 「无法归因」是诚实的一项：归因靠请求的出口 IP，而出口 IP 要先测速才知道，所以总有一部分
   请求认不出是哪个节点。这些请求单独计数，不会硬塞给某个节点凑一个好看的数字。
   反过来，这个数字异常大往往是有用的线索 —— 说明请求命中了规则却实际走了直连兜底，
   也就是代理根本没转发成功。
...
```bash
npm test      # 运行全部 271 个测试（node --test，零依赖）
