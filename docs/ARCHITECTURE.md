...
| **D22** | 重试只针对**代理层、连接层与限流失败**；HTTP 4xx / 5xx 一律不重试，**HTTP 429 例外** | 换个代理拿到的还是同一个 404，重发只是白给图源添一次请求；但 429 是“这个出口 IP 请求太密”，换一个出口正是这个扩展存在的理由，所以 `statusCode === 429` 单独归为 `rate-limit` 并可重试。`ERR_ABORTED` 只有内容脚本捕获到图片失败并来问时才进入重试（页面可见的取消才值得换节点），后台单独看到的导航取消不开启 unseen 计时。内容脚本只看得到「图裂了」，真正的原因在 `request-logger` 的失败原因表里（按 URL 暂存 30 秒）。查不到就保守放弃 —— `onErrorOccurred` 与渲染进程派发 `error` 没有顺序保证，宁可少救一张图 |
...
| **D28** | 「网络层失败了但页面没来问」计为 `retry.unseen`，由后台在宽限期（3 秒）后判定；**`ERR_ABORTED` 不参与这项计时** | 只有 DOM 里的 `<img>` 会派发可捕获的 error，而阅读器常用 `new Image()` 预加载 —— 那种失败内容脚本永远看不见。实测 13 次失败有 3 次属于此类，而这类失败以前**不产生任何计数**：既不是「重试了」也不是「判定为不重试」，于是面板「未重试」显示 0，读起来像「每次失败都重试了」。判定必须由后台做，因为页面根本没有机会开口。**两条路径没有顺序保证，所以要挡两个方向**：页面后到 → `planRetry` 入口撤销计时；页面先到 → 记一条时间戳，失败落地时看到它就不再计时（只挡后者会稳稳数出一个不存在的「没人来问」）。`ERR_ABORTED` 不计入 unseen：页面翻页/关页造成的取消不是“重试对这个站点整体无效”的结构性盲区，只有页面确实捕获到图片 error 时才由 retry 链重试。SW 被休眠吃掉个别计数是可接受的 —— 这一格要回答的是「重试对这个站点是不是整体无效」，是量级信号不是账本 |
...
| **D36** | 慢图看门狗：内容脚本捕获 `<img>` 的 `loadstart`，真实请求超过 `retry.slowTimeoutMs`（默认 12 秒）仍无结果时以 `cause:'slow'` 问后台并换节点 | 代理“慢但活着”不会派发 error，失败驱动重试（D20）整条链都碰不到它；测速只能量“连上代理要多久”，量不出“一张图多久才下得完”，而且节点多时串行测速本身就很慢。所以看门狗直接量真实图片请求的墙钟时间，并沿用后台同一套规则/次数/兜底判定（D21）。它覆盖普通 DOM 与 **open shadow root** 的 `<img>`（内容脚本把监听递归挂到 ShadowRoot 内部）；游离 `new Image()`、fetch/XHR 仍由深度重试（D31）覆盖；CSS 背景图、canvas 与 closed shadow root 依旧无解 —— closed shadow 需要阅读器自身开放。**自己换节点会中止旧请求，旧请求的 abort/error 必须压到新请求的 `loadstart` 之后**，否则一次切换会被错记成一次失败。统计里单独记 `retry.slow`，与失败驱动的 `attempted` 正交 |
...
③ ├─ 不匹配规则               → give-up，**不计数**（别人网站的裂图不该进你的统计）
  ├─ 原因是 404 / 5xx / 被拦截 / 原因不明
  │                          → give-up，计入 retry.skipped
  ├─ 原因是 429（限流）      → 与代理故障同一套判定（换出口正是解药）
  ├─ 原因是 ERR_ABORTED      → 能走到这里说明页面已捕获到图片 error；
  │                            按下面 attempt 的规则判定重试或兜底
...
Metrics {
  ...
  requests: {                  // 只统计命中了用户规则、且**真的走了网络**的请求
    total, ok, fail, aborted,  // aborted = 页面/用户/其他扩展取消（ERR_ABORTED / ERR_BLOCKED_*），不算失败
...
| `content/retry.js` | 页面侧执行端：捕获普通 DOM 与 open shadow root 里 `<img>` 的加载生命周期，为超时未完成的图启动看门狗（决策 D36）、捕获 error、问后台、重新赋值 `src`、回报结果。**classic script，不能有 import**（MV3 的 content_scripts 不支持 ESM） |
...
```bash
npm test    # 755 个测试（单元 + 集成 + 后台编排 + SW 冒烟 + 打包 + UI 契约）
npm run check
```
