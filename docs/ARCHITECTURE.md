...
```bash
npm test    # 195 个测试（单元 + 集成 + 后台编排 + SW 冒烟 + 打包 + UI 契约）
npm run check
```
...
| UI 契约 | `tests/ui-{tokens,contract,status}.test.js` | 静态解析 CSS/HTML/JS，无需 DOM 环境 |
| 打包产物 | `tests/pack.test.js` | 用另写一份的 zip 解析器把包拆回来逐字节比对 |

UI 这一层没有 DOM 环境，所以只测**能静态判定、且出错时浏览器不会报警**的东西：
...
- `ui-contract`：HTML 与 JS 之间只靠 id 字符串维系，改名不会报错、只会安静地
  ...
  可访问名、不出现内联样式，以及侧栏 hash 与 `options.js` 的分区表逐项对齐。
