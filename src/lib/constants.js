 ...
 * 图片加载看门狗的超时上限（毫秒）。
 *
 * 超过这个值的配置要么是手滑要么是写坏的：看门狗的目的是在「慢但没失败」时换节点，
 * 而不是等上一分钟才反应。0 表示关闭，这个上限只约束开启状态下的取值。
 */
export const RETRY_SLOW_TIMEOUT_CAP_MS = 60000;
...
export function defaultRetrySettings() {
  return {
    ...
    delayMs: 300,
    /**
     * 图片加载看门狗：DOM 里的 `<img>` 开始加载后，超过这个时长仍未收到 load/error
     * 就主动换一个节点重发。0 = 关闭。
     *
     * 与测速无关：探测只量得到「连上要多久」，量不到「一张图要多久才能下完」；
     * 而节点很多时串行测速本身就很慢。看门狗看的是真实图片请求的墙钟时间，
     * 不需要提前知道哪个节点慢 —— 慢的请求会在页面上自己暴露出来。
     */
    slowTimeoutMs: 12000,
  };
}
